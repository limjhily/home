...
라벨 행과 값 행이 분리돼 있어 "라벨 바로 뒤 숫자"를 읽으면 옆 칸 값을 집습니다.
그래서 파서는 **표 구조를 먼저 읽고**(`parse_tables`), 표를 못 찾을 때만 본문 텍스트로 넘어갑니다.
또 전매제한 5년 초과, 거주의무 5년 초과처럼 상식을 벗어난 값은 옆 칸 오독으로 보고 버립니다.
